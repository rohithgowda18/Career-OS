      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-text-muted">
            High-level snapshot of your application pipeline.
          </p>
        </div>
            ...
            {stats.allUpcomingDeadlines.length === 0 ? (
              ...
              <div className="space-y-3">
                {stats.allUpcomingDeadlines.map((app: any) => (
                  <div
                    key={app.id}
                    className="flex items-center justify-between p-4 rounded-xl bg-bg-elevated/40 border border-border group hover:border-primary/40 transition-all"
                  >
                    <div className="min-w-0 flex-1 mr-4">
                      <p className="font-bold text-sm text-text-main group-hover:text-primary transition-colors truncate">
                        {app.eventName}
                      </p>
            ...
            {stats.recentActivity.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {stats.recentActivity.map((app: any) => (
                  <div
                    key={app.id}
                    className="p-4 rounded-xl bg-bg-elevated/20 border border-border hover:border-primary/20 transition-all group"
                  >
                    ...
                    <p className="font-bold text-sm text-text-main truncate mb-1">
                      {app.eventName}
                    </p>
          ...
          <section className="card-premium p-6 sticky top-36">
                ...
                return (
                  <div key={status} className="group">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-[10px] font-black text-text-muted uppercase tracking-widest group-hover:text-text-main transition-colors">
                    ...
                    <div className="w-full h-1.5 rounded-full bg-bg-elevated border border-border overflow-hidden">
                      <div
                        className={cn(
                          "h-full transition-all duration-1000 shadow-[0_0_8px_rgba(249,115,22,0.4)]",
            ...
            <div className="mt-10 pt-6 border-t border-border flex flex-col items-center">
              ...
              <p className="text-4xl font-black text-primary mt-2 tabular-nums">
                {stats.totalApplications > 0
                  ? (
                      (stats.statusCounts.Accepted / stats.totalApplications) *
                      100
                    ).toFixed(0)
                  : 0}
                %
    ...
    <div className="card-premium p-6 group">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-3xl font-black text-text-main mt-3 group-hover:text-primary transition-colors tabular-nums">
            {value}
          </p>
        </div>
...
function EmptyState({ title, subtitle }: { title: string; subtitle: string }) {
  return (
    <div className="py-16 flex flex-col items-center justify-center text-center border-2 border-dashed border-border rounded-2xl bg-bg-elevated/5 group">
      ...
      <p className="text-sm font-bold text-text-muted uppercase tracking-widest mb-1">
        {title}
      </p>
