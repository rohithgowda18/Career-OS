import { Loader2, Calendar, TrendingUp, CheckCircle2, AlertCircle, ExternalLink } from "lucide-react";
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="card-elevated p-6">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-sm text-muted-foreground font-medium">Upcoming (7 days)</p>
              <p className="text-3xl font-bold text-foreground mt-2">{stats.upcomingDeadlines}</p>
            </div>
        ...
        <div className="card-elevated p-6">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-foreground mt-2">{stats.statusCounts.Accepted}</p>
            </div>
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-foreground mt-2">
                {stats.statusCounts.Applied + stats.statusCounts["Under Review"]}
              </p>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 card-elevated p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-foreground">Upcoming Deadlines (Next 7 Days)</h3>
            <Button
              size="sm"
          ...
          {stats.allUpcomingDeadlines.length === 0 ? (
            ...
            <div className="space-y-3">
              {stats.allUpcomingDeadlines.map((app) => (
                <div
                  key={app.id}
                ...
                >
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      {app.eventType} • {new Date(app.deadline!).toLocaleDateString()}
                    </p>
                  </div>
      ...
      <div className="card-elevated p-6">
        ...
        {stats.recentActivity.length === 0 ? (
          <div className="py-8 text-center">
            <p className="text-muted-foreground">No applications yet. Start by adding your first one!</p>
            <Button
              onClick={() => setShowAddModal(true)}
              className="mt-4"
            >
              ...
              >
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {app.eventType} • Added {new Date(app.createdAt).toLocaleDateString()}
                  </p>
                </div>
                <span className={`badge ${getStatusBadgeClass(app.status)}`}>
                  {app.status}
