package com.eventtracker.coding;

import com.eventtracker.coding.client.CodingPlatformClient;
import com.eventtracker.coding.client.dto.PlatformProfileData;
import com.eventtracker.coding.client.dto.PlatformStatsData;
import com.eventtracker.coding.dto.CodingStatsResponse;
import com.eventtracker.coding.dto.ConnectAccountRequest;
import com.eventtracker.coding.dto.ConnectAccountResponse;
import com.eventtracker.coding.entity.*;
import com.eventtracker.coding.repository.CodingAccountRepository;
import com.eventtracker.coding.repository.CodingStatsHistoryRepository;
import com.eventtracker.coding.repository.CodingStatsRepository;
import com.eventtracker.coding.service.CodingProfileService;
...
class CodingProfileServiceTest {
    ...
    void setUp() {
        ...
        service = new CodingProfileService(accountRepository, statsRepository, historyRepository, List.of(leetCodeClient));
    }
    ...
    @Test
    void testVerifyOwnership_Success() {
        CodingAccount account = CodingAccount.builder()
                .id(10L)
                .userId(1L)
                .platform(Platform.LEETCODE)
                .username("testcoder")
                .verificationCode("CAREER-1234")
                .verificationStatus(VerificationStatus.PENDING)
                .verificationExpiresAt(LocalDateTime.now().plusHours(5))
                .build();

        when(accountRepository.findByIdAndUserId(10L, 1L)).thenReturn(Optional.of(account));
        when(leetCodeClient.verifyOwnership("testcoder", "CAREER-1234")).thenReturn(true);
    ...
    void testVerifyOwnership_CodeMissing_Throws() {
        ...
        when(accountRepository.findByIdAndUserId(10L, 1L)).thenReturn(Optional.of(account));
        when(leetCodeClient.verifyOwnership("testcoder", "CAREER-1234")).thenReturn(false);
    ...
    @Test
    void testUnauthorizedAccountAccess_Throws() {
    ...
    @Test
    void testSyncStats_Success_CreatesHistory() {
        CodingAccount account = CodingAccount.builder()
                .id(10L)
                .userId(1L)
                .platform(Platform.LEETCODE)
