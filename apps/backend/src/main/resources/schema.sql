...
CREATE TABLE IF NOT EXISTS routine_completion (
    ...
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(routine_task_id, completion_date)
);

CREATE INDEX IF NOT EXISTS idx_routine_completion_date ON routine_completion(completion_date);
...
CREATE TABLE IF NOT EXISTS saved_jobs (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
    CONSTRAINT uq_saved_jobs_user_ext_source UNIQUE (user_id, external_job_id, source)
);

CREATE INDEX IF NOT EXISTS idx_saved_jobs_user_id ON saved_jobs(user_id);

-- CODING PROFILES (Phase 1: LeetCode)
