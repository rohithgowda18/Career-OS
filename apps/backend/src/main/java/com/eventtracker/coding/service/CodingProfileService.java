...
public class CodingProfileService {
    ...
    public CodingStatsResponse verifyOwnership(Long userId, Long accountId) {
        ...
        if (!isVerified) {
            throw new IllegalArgumentException("Verification code '" + account.getVerificationCode() + "' was not found in @" + account.getUsername() + "'s LeetCode profile bio (aboutMe). Please update your bio on LeetCode and try again.");
        }
    ...
    private String generateVerificationCode() {
        int randomInt = secureRandom.nextInt(0x10000);
        return String.format("CAREER-%04X", randomInt);
    }
